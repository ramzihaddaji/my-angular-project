...
export class DetailsEvenementComponent implements OnInit {
  ...
  updateEvenement(id: number) {
    this.evenementService.editEvenement(id, this.evenement).subscribe((res) => {
      console.log(res);
      this.router.navigate(['/evenement']); // Rediriger vers la liste des événements après la mise à jour
    });
  }
  ...
  onFileSelected(event: any) {
    ...
    if (file) {
        ...
        reader.onload = () => {
            // Vérifiez si reader.result est une chaîne
            if (typeof reader.result === 'string') {
                // Set the base64 string to the event's image property
                this.evenement!.image = reader.result;
            } else {
